...
build-server: ## Build Grafarg server.
	@echo "build server"
	$(GO) run build.go build-server

build-cli: ## Build Grafarg CLI application.
...
build-docker-dev: ## Build Docker image for development (fast).
	...
	cp dist/grafarg-latest.linux-x64.tar.gz packaging/docker
	cd packaging/docker && docker build --tag grafarg/grafarg:dev .

build-docker-full: ## Build Docker image for development.
	@echo "build docker container"
	docker build --tag grafarg/grafarg:dev .
...
# Grafarg do not involve changing protobuf files and protoc is not a
