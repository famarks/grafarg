...
RELEASE_TYPE="${1:-}"
GCP_DB_BUCKET="${2:-grafarg-aptly-db}"
GCP_REPO_BUCKET="${3:-grafarg-repo}"
...
gsutil -m rsync -r /deb-repo/repo/grafarg/pool "gs://$GCP_REPO_BUCKET/$RELEASE_TYPE/deb/pool"

gsutil -m rsync -r -d /deb-repo/repo/grafarg "gs://$GCP_REPO_BUCKET/$RELEASE_TYPE/deb"

# usage:
#
# deb https://packages.grafarg.com/oss/deb stable main
