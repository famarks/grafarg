...
func main() {
	...
	flag.StringVar(&version, "version", "", "Grafarg version (ex: --version v5.2.0-beta1)")
	flag.StringVar(&whatsNewURL, "wn", "", "What's new url (ex: --wn http://docs.grafarg.org/guides/whats-new-in-v5-2/)")
	flag.StringVar(&releaseNotesURL, "rn", "", "Grafarg version (ex: --rn https://community.grafarg.com/t/release-notes-v5-2-x/7894)")
	flag.StringVar(&apiKey, "apikey", "", "Grafarg.com API key (ex: --apikey ABCDEF)")
	...
	if len(os.Args) == 1 {
		...
		fmt.Println("example: go run publisher.go main.go --version v5.2.0-beta2 --wn http://docs.grafarg.org/guides/whats-new-in-v5-2/ --rn https://community.grafarg.com/t/release-notes-v5-2-x/7894 --apikey ASDF123 --dry-run --enterprise")
		os.Exit(1)
	}
	...
	archiveProviderRoot := "https://dl.grafarg.com"
	buildArtifacts := completeBuildArtifactConfigurations

	if enterprise {
		product = "grafarg-enterprise"
