...
import { dateTime, toUtc } from '@grafarg/data';
        ...
        targets: [
          {
            format: 'time_series',
            rawQuery: true,
            rawSql: 'select time, metric from grafarg_metric',
            refId: 'A',
            datasource: 'gdev-ds',
          },
        ],
      ...
      const data = {
        results: {
          A: {
            refId: 'A',
            meta: {
              executedQueryString: 'select time, metric from grafarg_metric',
              rowCount: 0,
            },
            series: [
              {
        ...
        targets: [
          {
            format: 'table',
            rawQuery: true,
            rawSql: 'select time, metric, value from grafarg_metric',
            refId: 'A',
            datasource: 'gdev-ds',
          },
        ],
      ...
      const data = {
        results: {
          A: {
            refId: 'A',
            meta: {
              executedQueryString: 'select time, metric, value from grafarg_metric',
              rowCount: 1,
            },
            series: null,
            tables: [
