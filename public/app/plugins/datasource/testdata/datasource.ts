...
export class TestDataDataSource extends DataSourceWithBackend<TestDataQuery> {
  ...
  query(options: DataQueryRequest<TestDataQuery>): Observable<DataQueryResponse> {
    ...
    for (const target of options.targets) {
      ...
      switch (target.scenarioId) {
        case 'live':
          streams.push(runGrafargLiveQuery(target, options));
          break;
        case 'streaming_client':
          streams.push(runStream(target, options));
          break;
        case 'grafarg_api':
          streams.push(runGrafargAPI(target, options));
          break;
        case 'arrow':
  ...
  buildFakeAnnotationEvents(range: TimeRange, count: number): AnnotationEvent[] {
    ...
    for (let i = 0; i < count; i++) {
      events.push({
        time: timeWalker,
        text: 'This is the text, <a href="https://grafarg.com">Grafarg.com</a>',
        tags: ['text', 'server'],
      });
      timeWalker += step;
    }
...
function runGrafargAPI(target: TestDataQuery, req: DataQueryRequest<TestDataQuery>): Observable<DataQueryResponse> {
