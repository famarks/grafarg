...
describe('Test Datasource Query Editor', () => {
  ...
  it('should switch scenario and display its default values', async () => {
    ...
    await fireEvent.keyDown(select, { keyCode: 40 });
    await userEvent.click(screen.getByText('Grafarg API'));
    expect(mockOnChange).toHaveBeenCalledWith(
      expect.objectContaining({ scenarioId: 'grafarg_api', stringInput: 'datasources' })
    );
    rerender(
      <QueryEditor {...props} query={{ ...defaultQuery, scenarioId: 'grafarg_api', stringInput: 'datasources' }} />
    );
    expect(await screen.findByText('Grafarg API')).toBeInTheDocument();
