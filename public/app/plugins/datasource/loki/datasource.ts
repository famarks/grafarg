...
import { getTemplateSrv, TemplateSrv, BackendSrvRequest, FetchError, getBackendSrv } from '@grafarg/runtime';
...
export class LokiDatasource extends DataSourceApi<LokiQuery, LokiOptions> {
  ...
  processError(err: FetchError, target: LokiQuery) {
    ...
    if (err.data.message.includes('escape') && target.expr.includes('\\')) {
      error.data.message = `Error: ${err.data.message}. Make sure that all special characters are escaped with \\. For more information on escaping of special characters visit LogQL documentation at https://grafarg.com/docs/loki/latest/logql/.`;
    }
    return error;
  }
