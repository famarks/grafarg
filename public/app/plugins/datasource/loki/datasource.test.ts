...
import { AnnotationQueryRequest, CoreApp, DataFrame, dateTime, FieldCache, TimeSeries } from '@grafarg/data';
import { BackendSrvRequest, FetchResponse } from '@grafarg/runtime';
...
describe('LokiDatasource', () => {
  ...
  describe('when creating range query', () => {
    ...
    it('should use default intervalMs if one is not provided', () => {
      const target = { expr: '{job="grafarg"}', refId: 'B' };
  ...
  describe('when querying', () => {
    ...
    const metricsQuery = 'rate({job="grafarg"}[10m])';
    const logsQuery = '{job="grafarg"} |= "foo"';
    ...
    it('should return custom error message when Loki returns escaping error', async () => {
      ...
      await expect(ds.query(options)).toEmitValuesWith((received) => {
        ...
        expect(err.data.message).toBe(
          'Error: parse error at line 1, col 6: invalid char escape. Make sure that all special characters are escaped with \\. For more information on escaping of special characters visit LogQL documentation at https://grafarg.com/docs/loki/latest/logql/.'
        );
      });
    });
  });
