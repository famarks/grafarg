...
import { deprecationWarning, ScopedVars, TimeRange } from '@grafarg/data';
import { getFilteredVariables, getVariables, getVariableWithName } from '../variables/state/selectors';
import { variableRegex } from '../variables/utils';
import { isAdHoc } from '../variables/guard';
import { VariableModel } from '../variables/types';
import { setTemplateSrv, TemplateSrv as BaseTemplateSrv } from '@grafarg/runtime';
...
export class TemplateSrv implements BaseTemplateSrv {
  ...
  setGrafargVariable(name: string, value: any) {
    this.grafargVariables[name] = value;
  }
  ...
  replace(target?: string, scopedVars?: ScopedVars, format?: string | Function): string {
    ...
    return target.replace(this.regex, (match, var1, var2, fmt2, var3, fieldPath, fmt3) => {
      ...
      const systemValue = this.grafargVariables[variable.current.value];
      if (systemValue) {
