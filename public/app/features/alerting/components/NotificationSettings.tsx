    ...
    <CollapsableSection label="Notification settings" isOpen={false}>
      ...
      {currentFormValues.uploadImage && !imageRendererAvailable && (
        <InfoBox title="No image renderer available/installed">
          Grafarg cannot find an image renderer to capture an image for the notification. Please make sure the Grafarg
          Image Renderer plugin is installed. Please contact your Grafarg administrator to install the plugin.
        </InfoBox>
      )}
      <Field>
        <Checkbox
