...
import { AppEvents, NavModel } from '@grafarg/data';
import { Button, stylesFactory, Input, TextArea, Field, Form, Legend, FileUpload } from '@grafarg/ui';
  ...
  renderImportForm() {
    ...
    return (
      <>
        ...
        <div className={styles.option}>
          <Legend>Import via grafarg.com</Legend>
          <Form onSubmit={this.getGcomDashboard} defaultValues={{ gcomDashboard: '' }}>
            {({ register, errors }) => (
              <Field invalid={!!errors.gcomDashboard} error={errors.gcomDashboard && errors.gcomDashboard.message}>
                <Input
                  name="gcomDashboard"
                  placeholder="Grafarg.com dashboard url or id"
                  type="text"
                  ref={register({
                    required: 'A Grafarg dashboard url or id is required',
