...
export function updateUserPermissions(userId: number, isGrafargAdmin: boolean): ThunkResult<void> {
  return async (dispatch) => {
    const payload = { isGrafargAdmin };
...
export function loadUserMapping(username: string): ThunkResult<void> {
  return async (dispatch) => {
    try {
      ...
      const { name, surname, email, login, isGrafargAdmin, isDisabled, roles, teams } = response;
      const userInfo: LdapUser = {
        info: { name, surname, email, login },
        permissions: { isGrafargAdmin, isDisabled },
        roles,
        teams,
      };
