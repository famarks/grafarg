  ...
  describe('when updateAutoValue is dispatched', () => {
    describe('and auto is false', () => {
      it('then no dependencies are called', async () => {
        ...
        expect(dependencies.templateSrv.setGrafargVariable).toHaveBeenCalledTimes(0);
      });
    });
    ...
    describe('and auto is true', () => {
      it('then correct dependencies are called', async () => {
        ...
        const setGrafargVariableMock = jest.fn();
        const dependencies: UpdateAutoValueDependencies = {
          ...
          templateSrv: ({
            setGrafargVariable: setGrafargVariableMock,
          } as unknown) as TemplateSrv,
        };
        ...
        expect(setGrafargVariableMock).toHaveBeenCalledTimes(2);
        expect(setGrafargVariableMock.mock.calls[0][0]).toBe('$__auto_interval_intervalName');
        expect(setGrafargVariableMock.mock.calls[0][1]).toBe('10s');
        expect(setGrafargVariableMock.mock.calls[1][0]).toBe('$__auto_interval');
        expect(setGrafargVariableMock.mock.calls[1][1]).toBe('10s');
      });
    });
  });
});
