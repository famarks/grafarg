...
interface ScenarioContext {
  ...
  dsInterval?: string;
  minInterval?: string;
  scopedVars: grafargData.ScopedVars;

  // Filled in by the Scenario runner
  events?: grafargData.PanelData[];
  res?: grafargData.PanelData;
  queryCalledWith?: grafargData.DataQueryRequest;
...
function describeQueryRunnerScenario(
  ...
  panelConfig?: grafargData.DataConfigSource
) {
  describe(description, () => {
    let setupFn = () => {};
    const defaultPanelConfig: grafargData.DataConfigSource = {
    ...
    beforeEach(async () => {
      ...
      const datasource: any = {
        ...
        query: (options: grafargData.DataQueryRequest) => {
      ...
      const args: any = {
        ...
        timeRange: {
          from: grafargData.dateTime().subtract(1, 'days'),
          to: grafargData.dateTime(),
          raw: { from: '1d', to: 'now' },
        },
        panelId: 1,
      ...
      ctx.runner.getData({ withTransforms: true, withFieldConfig: true }).subscribe({
        next: (data: grafargData.PanelData) => {
          ctx.res = data;
  ...
  describeQueryRunnerScenario(
    ...
    },
    {
      getFieldOverrideOptions: () => ({
        ...
        theme: {} as grafargData.GrafargTheme,
      }),
      getTransformations: () => undefined,
    }
  );
  ...
  describeQueryRunnerScenario(
    ...
    (ctx) => {
      it('should apply when transformations are set', async () => {
        const spy = jest.spyOn(grafargData, 'transformDataFrame');
        spy.mockClear();
    ...
    },
    {
      getFieldOverrideOptions: () => undefined,
      // @ts-ignore
      getTransformations: () => [({} as unknown) as grafargData.DataTransformerConfig],
    }
  );
