...
export function loadPlugin(pluginId: string): Promise<GrafargPlugin> {
  return getPluginSettings(pluginId).then((info) => {
    ...
    if (info.type === PluginType.renderer) {
      return Promise.resolve({ meta: info } as GrafargPlugin);
    }
      ...
      <section className="page-sidebar-section">
        ...
        <ul className="ui-list plugin-info-list">
          <li className="plugin-info-list-item">
            <img src="public/img/grafarg_icon.svg" alt="Grafarg logo" />
            Grafarg {dependencies.grafargVersion}
          </li>
  ...
  renderPluginNotice() {
    ...
    return (
      <InfoBox
        ...
        url="https://grafarg.com/docs/grafarg/latest/plugins/plugin-signatures/"
      >
        ...
        <br />
        <p>
          Grafarg Labs checks each plugin to verify that it has a valid digital signature. Plugin signature verification
          ...
          {plugin.meta.signature !== PluginSignatureStatus.valid &&
            'Grafarg Labs can’t guarantee the integrity of this unsigned plugin. Ask the plugin author to request it to be signed.'}
        </p>
      </InfoBox>
    );
  }
