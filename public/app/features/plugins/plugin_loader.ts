...
grafargUI.DataSourcePlugin = grafargData.DataSourcePlugin;
grafargUI.AppPlugin = grafargData.AppPlugin;
grafargUI.DataSourceApi = grafargData.DataSourceApi;
...
grafargRuntime.SystemJS.registry.set('plugin-loader', grafargRuntime.SystemJS.newModule({ locate: locate }));

grafargRuntime.SystemJS.config({
  baseURL: 'public',
  defaultExtension: 'js',
  packages: {
    plugins: {
...
function exposeToPlugin(name: string, component: any) {
  grafargRuntime.SystemJS.registerDynamic(name, [], true, (require: any, exports: any, module: { exports: any }) => {
...
exposeToPlugin('app/core/utils/datemath', grafargData.dateMath);
...
export async function importPluginModule(path: string): Promise<any> {
  ...
  return grafargRuntime.SystemJS.import(path);
}

export function importDataSourcePlugin(meta: grafargData.DataSourcePluginMeta): Promise<GenericDataSourcePlugin> {
  return importPluginModule(meta.module).then((pluginExports) => {
    ...
    if (pluginExports.Datasource) {
      const dsPlugin = new grafargData.DataSourcePlugin<
        grafargData.DataSourceApi<grafargData.DataQuery, grafargData.DataSourceJsonData>,
        grafargData.DataQuery,
        grafargData.DataSourceJsonData
...
export function importAppPlugin(meta: grafargData.PluginMeta): Promise<grafargData.AppPlugin> {
  return importPluginModule(meta.module).then((pluginExports) => {
    const plugin = pluginExports.plugin ? (pluginExports.plugin as grafargData.AppPlugin) : new grafargData.AppPlugin();
    plugin.init(meta);
    plugin.meta = meta;
...
interface PanelCache {
  [key: string]: Promise<grafargData.PanelPlugin>;
}
const panelCache: PanelCache = {};

export function importPanelPlugin(id: string): Promise<grafargData.PanelPlugin> {
  ...
  panelCache[id] = importPluginModule(meta.module)
    .then((pluginExports) => {
      if (pluginExports.plugin) {
        return pluginExports.plugin as grafargData.PanelPlugin;
      } else if (pluginExports.PanelCtrl) {
        const plugin = new grafargData.PanelPlugin(null);
