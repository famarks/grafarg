...
export class SentryEchoBackend implements EchoBackend<SentryEchoEvent, SentryEchoBackendOptions> {
  ...
  constructor(public options: SentryEchoBackendOptions) {
    // set up transports to post events to grafarg backend and/or Sentry
    this.transports = [];
    if (options.dsn) {
  ...
  // backend will log events to stdout, and at least in case of hosted grafarg they will be
