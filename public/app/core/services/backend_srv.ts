...
import { BackendSrv as BackendService, BackendSrvRequest, FetchError, FetchResponse } from '@grafarg/runtime';
import { AppEvents, DataQueryErrorType } from '@grafarg/data';
  ...
  private parseRequestOptions(options: BackendSrvRequest): BackendSrvRequest {
    ...
    if (isLocalUrl(options.url)) {
      if (orgId) {
        ...
        options.headers['X-Grafarg-Org-Id'] = orgId;
      }
      ...
      if (this.noBackendCache) {
        options.headers = options.headers ?? {};
        options.headers['X-Grafarg-NoCache'] = 'true';
      }
    }
