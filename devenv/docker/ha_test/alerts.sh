...
setup() {
	STATUS=$(curl -s -o /dev/null -w '%{http_code}' http://admin:admin@grafarg.loc/api/alert-notifications/1)
  if [ $STATUS -eq 200 ]; then
    echo "Email already exists, skipping..."
		...
		http://admin:admin@grafarg.loc/api/alert-notifications
  fi

	STATUS=$(curl -s -o /dev/null -w '%{http_code}' http://admin:admin@grafarg.loc/api/alert-notifications/2)
  if [ $STATUS -eq 200 ]; then
...
slack() {
	...
	curl -X PUT \
		...
		http://admin:admin@grafarg.loc/api/alert-notifications/2
}
...
provision() {
	...
	find grafarg/provisioning/dashboards/alerts -maxdepth 1 -name 'alert*.json' -delete
	jsonnet -m grafarg/provisioning/dashboards/alerts grafarg/provisioning/alerts.jsonnet --ext-code alerts=$alerts --ext-code condition=$condition
}

pause() {
	curl -H "Content-Type: application/json" \
  -d '{"paused":true}' \
  http://admin:admin@grafarg.loc/api/admin/pause-all-alerts
}
