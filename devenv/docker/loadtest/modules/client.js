...
export const GrafargClient = class GrafargClient {
  constructor(httpClient) {
  ...
  onBeforeRequest(params) {
    if (this.orgId && this.orgId > 0) {
      ...
      params.headers['X-Grafarg-Org-Id'] = this.orgId;
    }
  }
};
...
export const createClient = (url) => {
  return new GrafargClient(new BaseClient(url, ''));
};

export const createBasicAuthClient = (url, username, password) => {
  return new GrafargClient(new BasicAuthClient(url, '', username, password));
};

export const createBearerAuthClient = (url, token) => {
  return new GrafargClient(new BearerAuthClient(url, '', token));
};
