...
func TestLoadingSettings(t *testing.T) {
	...
	Convey("Test reading string values from .ini file", t, func() {
		...
		Convey("If key is found - should return value from ini file", func() {
			...
			So(value, ShouldEqual, "https://grafarg.com/")
		})
...
func TestParseAppURLAndSubURL(t *testing.T) {
	testCases := []struct {
		...
		{rootURL: "http://localhost:3000", expectedAppURL: "http://localhost:3000/"},
		{rootURL: "http://localhost:3000/grafarg", expectedAppURL: "http://localhost:3000/grafarg/", expectedAppSubURL: "/grafarg"},
		{rootURL: "http://localhost:3000/grafarg/", expectedAppURL: "http://localhost:3000/grafarg/", expectedAppSubURL: "/grafarg"},
	}
...
func TestGetCDNPath(t *testing.T) {
	...
	cfg.CDNRootURL, err = url.Parse("http://cdn.grafarg.com")
	require.NoError(t, err)

	require.Equal(t, "http://cdn.grafarg.com/grafarg-oss/v7.5.0-11124/", cfg.GetContentDeliveryURL("grafarg-oss"))
	require.Equal(t, "http://cdn.grafarg.com/grafarg/v7.5.0-11124/", cfg.GetContentDeliveryURL("grafarg"))
}

func TestGetContentDeliveryURLWhenNoCDNRootURLIsSet(t *testing.T) {
	cfg := NewCfg()
	require.Equal(t, "", cfg.GetContentDeliveryURL("grafarg-oss"))
}
...
func TestGetCDNPathWithPreReleaseVersionAndSubPath(t *testing.T) {
	...
	cfg.CDNRootURL, err = url.Parse("http://cdn.grafarg.com/sub")
	require.NoError(t, err)
	require.Equal(t, "http://cdn.grafarg.com/sub/grafarg-oss/pre-releases/v7.5.0-11124pre/", cfg.GetContentDeliveryURL("grafarg-oss"))
	require.Equal(t, "http://cdn.grafarg.com/sub/grafarg/pre-releases/v7.5.0-11124pre/", cfg.GetContentDeliveryURL("grafarg"))
}
...
func TestGetCDNPathWithAlphaVersion(t *testing.T) {
	...
	cfg.CDNRootURL, err = url.Parse("http://cdn.grafarg.com")
	require.NoError(t, err)
	require.Equal(t, "http://cdn.grafarg.com/grafarg-oss/pre-releases/v7.5.0-alpha.11124/", cfg.GetContentDeliveryURL("grafarg-oss"))
	require.Equal(t, "http://cdn.grafarg.com/grafarg/pre-releases/v7.5.0-alpha.11124/", cfg.GetContentDeliveryURL("grafarg"))
}
