...
func Test_parseRedisConnStr(t *testing.T) {
	cases := map[string]struct {
		...
		"all redis options should parse": {
			"addr=127.0.0.1:6379,pool_size=100,db=1,password=grafargRocks,ssl=false",
			&redis.Options{
				Addr:      "127.0.0.1:6379",
				PoolSize:  100,
				DB:        1,
				Password:  "grafargRocks",
				Network:   "tcp",
				TLSConfig: nil,
			},
			false,
		...
		"ssl set to true should result in default TLS configuration with tls set to addr's host": {
			"addr=grafarg.com:6379,ssl=true",
			&redis.Options{
				Addr:      "grafarg.com:6379",
				Network:   "tcp",
				TLSConfig: &tls.Config{ServerName: "grafarg.com"},
			},
			false,
		},
