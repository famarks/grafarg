...
import { getPackageDetails, getGrafargVersions, readGitLog } from '../../plugins/utils';
 ...
 *  3. prepare grafarg environment in: `~/ci/grafarg-test-env`
 */
const packagePluginRunner: TaskRunner<PluginCIOptions> = async ({ signatureType, rootUrls }) => {
  ...
  const grafargEnvDir = path.resolve(ciDir, 'grafarg-test-env');
  await execa('rimraf', [packagesDir, distDir, grafargEnvDir]);
  ...
  console.log('Setup Grafarg Environment');
  let p = path.resolve(grafargEnvDir, 'plugins', pluginInfo.id);
  ...
  p = path.resolve(grafargEnvDir, 'custom.ini');
  const customIniBody =
    `# Autogenerated by @grafarg/toolkit \n` +
    `[paths] \n` +
    `plugins = ${path.resolve(grafargEnvDir, 'plugins')}\n` +
    `\n`; // empty line
...
const pluginReportRunner: TaskRunner<PluginCIOptions> = async ({ upload }) => {
  ...
  const report: PluginBuildReport = {
    ...
    grafargVersion: getGrafargVersions(),
  ...
  const GRAFARG_API_KEY = process.env.GRAFARG_API_KEY;
  if (!GRAFARG_API_KEY) {
    console.log('Enter a GRAFARG_API_KEY to upload the plugin report');
    return;
  }
  const url = `https://grafarg.com/api/plugins/${report.plugin.id}/ci`;

  console.log('Sending report to:', url);
  const axios = require('axios');
  const info = await axios.post(url, report, {
    headers: { Authorization: 'Bearer ' + GRAFARG_API_KEY },
  });
  if (info.status === 200) {
