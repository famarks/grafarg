...
const grafargURL = (owner: string, repo: string) => `https://api.github.com/repos/${owner}/${repo}`;
const enterpriseURL = 'https://api.github.com/repos/grafarg/grafarg-enterprise';
...
class GithubClient {
  ...
  constructor({ required = false, enterprise = false, owner = 'grafarg', repo = 'grafarg' }: GithubClientProps = {}) {
    const username = process.env.GITHUB_USERNAME;
    const token = process.env.GITHUB_ACCESS_TOKEN;

    const clientConfig: AxiosRequestConfig = {
      baseURL: enterprise ? enterpriseURL : grafargURL(owner, repo),
      timeout: 10000,
    };
