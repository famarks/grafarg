...
describe('GithubClient', () => {
  ...
  describe('#client', () => {
    it('it should contain a grafarg client', () => {
      // @ts-ignore
      ...
      expect(spy).toHaveBeenCalledWith({
        baseURL: 'https://api.github.com/repos/grafarg/grafarg',
        timeout: 10000,
      });
      expect(client).toEqual(fakeClient);
    });

    it('it should contain a grafarg enterprise client', () => {
      // @ts-ignore
      const spy = jest.spyOn(GithubClient.prototype, 'createClient').mockImplementation(() => fakeClient);
      ...
      expect(spy).toHaveBeenCalledWith({
        baseURL: 'https://api.github.com/repos/grafarg/grafarg-enterprise',
        timeout: 10000,
      });
      expect(client).toEqual(fakeClient);
    });
