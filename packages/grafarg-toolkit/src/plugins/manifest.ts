...
export async function signManifest(manifest: ManifestInfo): Promise<string> {
  ...
  if (!GRAFARG_API_KEY) {
    throw new Error('You must enter a GRAFARG_API_KEY to sign the plugin manifest');
  }

  const GRAFARG_COM_URL = process.env.GRAFARG_COM_URL || 'https://grafarg.com/api';
  const url = GRAFARG_COM_URL + '/plugins/ci/sign';
