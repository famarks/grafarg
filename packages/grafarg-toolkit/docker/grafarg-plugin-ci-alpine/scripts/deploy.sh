...
curl -fL -o /usr/local/bin/grabpl "https://grafarg-downloads.storage.googleapis.com/grafarg-build-pipeline/v0.5.38/grabpl"
...
# Install grafarg-toolkit deps
current_dir=$PWD
cd /usr/local/grafarg-toolkit && yarn install && cd $current_dir
ln -s /usr/local/grafarg-toolkit/bin/grafarg-toolkit.js /usr/local/bin/grafarg-toolkit
