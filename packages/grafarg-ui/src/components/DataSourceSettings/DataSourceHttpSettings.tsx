...
const HttpAccessHelp = () => (
  <div className="grafarg-info-box m-t-2">
    <p>
    ...
    <p>
      All requests will be made from the browser to Grafarg backend/server which in turn will forward the requests to
      the data source and by that circumvent possible Cross-Origin Resource Sharing (CORS) requirements. The URL needs
      to be accessible from the grafarg backend/server if you select this access mode.
    </p>
...
export const DataSourceHttpSettings: React.FC<HttpSettingsProps> = (props) => {
  ...
  switch (dataSourceConfig.access) {
    ...
    case 'proxy':
      urlTooltip = (
        <>
          Your access method is <em>Server</em>, this means the URL needs to be accessible from the grafarg
          backend/server.
        </>
      );
      break;
