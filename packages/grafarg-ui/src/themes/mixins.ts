...
export function hoverColor(color: string, theme: GrafargTheme): string {
  return theme.isDark ? tinycolor(color).brighten(2).toString() : tinycolor(color).darken(2).toString();
}

export function listItem(theme: GrafargTheme): string {
  return `
  background: ${theme.colors.bg2};
  &:hover {
    background: ${hoverColor(theme.colors.bg2, theme)};
...
export function listItemSelected(theme: GrafargTheme): string {
  return `
       background: ${hoverColor(theme.colors.bg2, theme)};
...
export const focusCss = (theme: GrafargTheme) => `
