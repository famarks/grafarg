...
import { grafargDataFrameToArrowTable, arrowTableToDataFrame } from './ArrowDataFrame';
...
describe('Read/Write arrow Table to DataFrame', () => {
  test('should parse output with dataframe', () => {
    ...
    const table = grafargDataFrameToArrowTable(frame);
  ...
  test('should support duplicate field names', () => {
    ...
    const table = grafargDataFrameToArrowTable(frame, true);
    expect(table.length).toEqual(frame.length);
  ...
  test('Export arrow table names', () => {
    ...
    const t1 = grafargDataFrameToArrowTable(simple);
    const t2 = grafargDataFrameToArrowTable(simple, true);
