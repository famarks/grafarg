...
let colorsMapTheme: GrafargTheme | undefined;
...
export function buildColorsMapForTheme(theme: GrafargTheme): Record<Color, string> {
  theme = theme ?? GrafargThemeType.Dark;
...
export function getColorForTheme(color: string, theme: GrafargTheme): string {
  if (!color) {
    return 'gray';
  }
 ...
 */
export function getColorFromHexRgbOrName(color: string, type?: GrafargThemeType): string {
  const themeType = type ?? GrafargThemeType.Dark;

  if (themeType === GrafargThemeType.Dark) {
    const darkTheme = ({
      type: themeType,
      colors: {
        panelBg: '#141619',
