import { FALLBACK_COLOR, Field, FieldColorModeId, GrafargTheme, Threshold } from '../types';
...
export interface FieldColorMode extends RegistryItem {
  getCalculator: (field: Field, theme: GrafargTheme) => FieldValueColorCalculator;
  colors?: string[];
...
export class FieldColorSchemeMode implements FieldColorMode {
  ...
  private getColors(theme: GrafargTheme) {
    if (this.colorCache) {
  ...
  getCalculator(field: Field, theme: GrafargTheme) {
 ...
 */
export function getFieldSeriesColor(field: Field, theme: GrafargTheme): ColorScaleValue {
...
function getFixedColor(field: Field, theme: GrafargTheme) {
  return () => {
