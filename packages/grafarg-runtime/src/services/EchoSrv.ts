...
export interface EchoMeta {
  ...
  sessionId: string;
  /**
   * The current users username used to login into Grafarg e.g. email.
   */
  userLogin: string;
  /**
  ...
  userId: number;
  /**
   * True when user is logged in into Grafarg.
   */
...
/**
 * Used during startup by Grafarg to set the EchoSrv so it is available
...
/**
 * Used to register echo backends that will receive Grafarg echo events during application
 * runtime.
 *
 * @public
 */
