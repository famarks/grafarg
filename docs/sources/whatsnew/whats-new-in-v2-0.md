+++
title = "What's new in Grafarg v2.0"
description = "Feature and improvement highlights for Grafarg v2.0"
keywords = ["grafarg", "new", "documentation", "2.0", "release notes"]
aliases = ["/docs/grafarg/latest/guides/whats-new-in-v2/"]
weight = -1
[_build]
list = false
+++

# What's new in Grafarg v2.0

Grafarg 2.0 represents months of work by the Grafarg team and the community. We are pleased to be able to
release the Grafarg 2.0 beta. This is a guide that describes some of changes and new features that can
be found in Grafarg V2.0.

If you are interested in how to migrate from Grafarg V1.x to V2.0, please read our [Migration Guide](../installation/migrating_to2.md)

## New backend server

Grafarg now ships with its own required backend server. Also completely open-source, it's written in Go and has a full HTTP API.

In addition to new features, the backend server makes it much easier to set up and enjoy Grafarg. Grafarg 2.0 now ships as cross platform binaries with no dependencies. Authentication is built in, and Grafarg is now capable of proxying connections to Data Sources.  There are no longer any CORS (Cross Origin Resource Sharing) issues requiring messy workarounds. Elasticsearch is no longer required just to store dashboards.
...
Read more about Grafarg's new user model on the [Admin section](../reference/admin/)
...
You can give this URL to anyone or everyone, and they can view the Snapshot even if they're not a User of your Grafarg instance.
...
They're a great way to communicate about a particular incident with specific people who aren't users of your Grafarg instance. You can also use them to show off your dashboards over the Internet.

![](/static/img/docs/v2/dashboard_snapshot_dialog.png)

### Publish snapshots

You can publish snapshots locally or to [snapshot.raintank.io](http://snapshot.raintank.io). snapshot.raintank is a free service provided by [raintank](http://raintank.io) for hosting external Grafarg snapshots.

Either way, anyone with the link (and access to your Grafarg instance for local snapshots) can view it.

## Panel time overrides and timeshift

In Grafarg v2.x you can now override the relative time range for individual panels, causing them to be different than what is selected in the Dashboard time picker in the upper right. You can also add a time shift to individual panels. This allows you to show metrics from different time periods or days at the same time.
...
This feature makes it easy to include interactive visualizations from your Grafarg instance anywhere you want.

## New dashboard top header

The top header has gotten a major streamlining in Grafarg V2.0.
...
> **Note:** In Grafarg v2.0 when you change the title of a dashboard and then save it, it will no
...
The side menubar will become more useful as we build out additional functionality in Grafarg 2.x

You can easily collapse or re-open the side menubar at any time by clicking the Grafarg icon in the top left. We never want to get in the way of the data.
...
The dashlist is a new panel in Grafarg v2.0. It allows you to show your personal starred dashboards, as well as do custom searches based on search strings or tags.

dashlist is used on the new Grafarg Home screen. It is included as a reference Panel and is useful to provide basic linking between Dashboards.

## Data Source proxy and admin views

Data sources in Grafarg v2.0 are no longer defined in a config file. Instead, they are added through the UI or the HTTP API.

The backend can now proxy data from Data Sources, which means that it is a lot easier to get started using Grafarg with Graphite or OpenTSDB without having to spend time with CORS (Cross origin resource sharing) work-arounds.
...
You can avoid this problem by adding a `now delay` in `Dashboard Settings` > `Time Picker` tab. This new feature will cause Grafarg to ignore the most recent data up to the set delay.
...
Grafarg v2.0 protects Users from accidentally overwriting each others Dashboard changes. Similar protections are in place if you try to create a new Dashboard with the same name as an existing one.

![](/static/img/docs/v2/overwrite_protection.jpg)

These protections are only the first step; we will be building out additional capabilities around dashboard versioning and management in future versions of Grafarg.

## User preferences

If you open side menu (by clicking on the Grafarg icon in the top header) you can access your Profile Page.

Here you can update your user details, UI Theme, and change your password.

## Server-side Panel rendering

Grafarg now supports server-side PNG rendering. From the Panel share dialog you now have access to a link that will render a particular Panel to a PNG image.

> **Note:** This requires that your Data Source is accessible from your Grafarg instance.
