...
keywords = ["grafarg", "docs", "stat panel"]
aliases = ["/docs/grafarg/latest/features/panels/stat/", "/docs/grafarg/latest/features/panels/singlestat/", "/docs/grafarg/latest/reference/singlestat/"]
weight = 900
+++
...
> **Note:** This panel replaces the Singlestat panel, which was deprecated in Grafarg 7.0.
...
- **Show -** Choose how Grafarg displays your data.
...
- **Value -** Select a reducer function that Grafarg will use to reduce many fields to a single value. Click the **Value** list to see functions and brief descriptions.
- **Orientation -** Choose a stacking direction.
  - **Auto -** Grafarg selects what it thinks is the best orientation.
  - **Horizontal -** Bars stretch horizontally, left to right.
  - **Vertical -** Bars stretch vertically, top to bottom.
- **Text mode -** (Only available in Grafarg 7.1+.) You can use the Text mode option to control what text the panel renders. If the value is not important, only the name and color is, then change the **Text mode** to **Name**. The value will still be used to determine color and is displayed in a tooltip.
