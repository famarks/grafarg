...
Grafarg automatically assigns Y-axis to the series. When there are two or more series with different units, then Grafarg assigns the left axis to the first unit and right to the following units.
...
Set a fixed width of the axis. By default, Grafarg dynamically calculates the width of an axis.
...
Set a **Soft min** or **soft max** option for better control of Y-axis limits. By default, Grafarg sets the range for the Y-axis automatically based on the dataset.
...
For examples, refer to the Grafarg Play dashboard [New Features in v7.4](https://play.grafarg.org/d/nP8rcffGk/new-features-in-v7-4?orgId=1).
