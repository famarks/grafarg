+++
title = "Table panel"
keywords = ["grafarg", "dashboard", "documentation", "panels", "table panel"]
aliases = ["/docs/grafarg/latest/reference/table/", "/docs/grafarg/latest/features/panels/table_panel/"]
weight = 1000
+++
...
> **Note:** If you are using a table visualization created before Grafarg 7.0, then you need to migrate to the new table version in order to see these options. To migrate, on the Panel tab, click **Table** visualization. Grafarg updates the table version and you can then access all table options.
