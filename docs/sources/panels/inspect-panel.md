...
> **Note:** This documentation refers to a feature only available in Grafarg 7.0+.

The panel inspector helps you understand and troubleshoot your panels. You can inspect the raw data for any Grafarg panel, export that data to a comma-separated values (CSV) file, view query requests, and export panel and data JSON.
...
- **JSON tab -** Allows you to view and copy the panel JSON, panel data JSON, and data frame structure JSON. This is useful if you are provisioning or administering Grafarg.
- **Query tab -** Shows you the requests to the server sent when Grafarg queries the data source.
...
1. In Grafarg, navigate to the dashboard that contains the panel you want to inspect.
...
Grafarg generates a CSV file in your default browser download location. You can open it in the viewer of your choice.
...
Grafarg sends a query to the server to collect information and then displays the result. You can now drill down on specific portions of the query, expand or collapse all of it, or copy the data to the clipboard to use in other applications.
