...
The primary use case for expressions is for the upcoming next version of Grafarg alerting. Like alerting, processing is done server-side, so expressions can operate without a browser session. However, expressions can be used with backend data sources and visualization as well.

> **Note:** Expressions do not work with current Grafarg alerting.
...
> **Note:** When possible, you should do data processing inside the data source. Copying data from storage to the Grafarg server for processing is inefficient, so expressions are targeted at lightweight data processing.
