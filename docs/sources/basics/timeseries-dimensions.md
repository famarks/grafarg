...
keywords = ["grafarg", "intro", "guide", "concepts", "timeseries", "labels"]
aliases = ["/docs/grafarg/latest/guides/timeseries-dimensions", "/docs/grafarg/latest/getting-started/timeseries-dimensions"]
weight = 600
+++
...
With time series data, the data often contain more than a single series, and is a set of multiple time series. Many Grafarg data sources support this type of data.
...
> **Note:** Multiple dimensions are not supported in a way that maps to multiple alerts in Grafarg, but rather they are treated as multiple conditions to a single alert. See the documentation on [creating alerts with multiple series]({{< relref "../alerting/create-alerts.md#multiple-series" >}}).
