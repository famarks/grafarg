...
The plugin.json file is required for all plugins. When Grafarg starts, it scans the plugin folders and mounts every folder that contains a plugin.json file unless the folder contains a subfolder named dist. In that case, Grafarg mounts the dist folder instead.
...
| `id`            | string                  | **Yes**  | Unique name of the plugin. If the plugin is published on grafarg.com, then the plugin id has to follow the naming conventions.                                                                                                                                                                                                                                                                          |
| `info`          | [object](#info)         | **Yes**  | Metadata for the plugin. Some fields are used on the plugins page in Grafarg and others on grafarg.com if the plugin is published.                                                                                                                                                                                                                                                                      |
...
| `executable`    | string                  | No       | The first part of the file name of the backend component executable. There can be multiple executables built for different operating system and architecture. Grafarg will check for executables named `<executable>_<$GOOS>_<lower case $GOARCH><.exe for Windows>`, e.g. `plugin_linux_amd64`. Combination of $GOOS and $GOARCH can be found here: https://golang.org/doc/install/source#environment. |
...
| Property            | Type                 | Required | Description                                                                                                                   |
|---------------------|----------------------|----------|-------------------------------------------------------------------------------------------------------------------------------|
| `grafargDependency` | string               | **Yes**  | Required Grafarg version for this plugin. Validated using https://github.com/npm/node-semver.                                 |
| `grafargVersion`    | string               | No       | (Deprecated) Required Grafarg version for this plugin, e.g. `6.x.x 7.x.x` to denote plugin requires Grafarg v6.x.x or v7.x.x. |
| `plugins`           | [object](#plugins)[] | No       | An array of required plugins on which this plugin depends.                                                                    |

### plugins

Plugin dependency. Used to display information about plugin dependencies in the Grafarg UI.
...
| `icon`       | string  | No       | Icon to use in the side menu. For information on available icon, refer to [Icons Overview](https://developers.grafarg.com/ui/latest/index.html?path=/story/docs-overview-icon--icons-overview). |
| `name`       | string  | No       |                                                                                                                                                                                                 |
...
Metadata for the plugin. Some fields are used on the plugins page in Grafarg and others on grafarg.com if the plugin is published.

### Properties

| Property      | Type                     | Required | Description                                                                                                                   |
|---------------|--------------------------|----------|-------------------------------------------------------------------------------------------------------------------------------|
| `keywords`    | string[]                 | **Yes**  | Array of plugin keywords. Used for search on grafarg.com.                                                                     |
...
| `description` | string                   | No       | Description of plugin. Used on the plugins page in Grafarg and for search on grafarg.com.                                     |
