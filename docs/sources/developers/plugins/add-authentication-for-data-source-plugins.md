...
aliases = ["/docs/grafarg/latest/plugins/developing/auth-for-datasources/", "/docs/grafarg/latest/developers/plugins/authentication/"]
+++

# Add authentication for data source plugins

This page explains how to use the Grafarg data source proxy to authenticate against a third-party API from a data source plugin.

When a user saves a password or any other sensitive data as a data source option, Grafarg encrypts the data and stores it in the Grafarg database. Any encrypted data source options can only be decrypted on the Grafarg server. This means that any data source that makes authenticated queries needs to request the decrypted data to be sent to the browser.

To minimize the amount of sensitive information sent to and from the browser, data source plugins can use the Grafarg _data source proxy_. When using the data source proxy, any requests containing sensitive information go through the Grafarg server. No sensitive data is sent to the browser after the data is saved.
...
To forward requests through the Grafarg proxy, you need to configure one or more _routes_. A route specifies how the proxy transforms outgoing requests. All routes for a given plugin are defined in the [plugin.json]({{< relref "metadata.md" >}}) file.

Let's add a route to proxy requests to `https://api.example.com/foo/bar`.

1. Add the route to `plugin.json`. Note that you need to reload the Grafarg server every time you make a change to your `plugin.json` file.
...
The Grafarg proxy supports a number of different authentication methods. For more information on how to configure each authentication method, refer to [plugin.json]({{< relref "metadata.md" >}}).
...
Grafarg automatically renews the token when it expires.
