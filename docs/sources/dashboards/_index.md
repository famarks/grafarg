+++
title = "Dashboards"
aliases = ["/docs/grafarg/latest/features/dashboard/dashboards/"]
weight = 80
+++

# Dashboard overview

A _dashboard_ is a set of one or more panels organized and arranged into one or more rows. Grafarg ships with a variety of Panels. Grafarg makes it easy to construct the right queries, and customize the display properties so that you can create the perfect dashboard for your need. Each panel can interact with data from any configured Grafarg Data Source (currently Graphite, Prometheus, Elasticsearch, InfluxDB, OpenTSDB, MySQL, PostgreSQL, Microsoft SQL Server and AWS Cloudwatch).
...
We use a unit abstraction so that Grafarg looks great on all screen sizes.

 > **Note:** With MaxDataPoint functionality, Grafarg can show you the perfect number of data points, regardless of resolution or time range.
