+++
title = "JSON model"
keywords = ["grafarg", "dashboard", "documentation", "json", "model"]
aliases = ["/docs/grafarg/latest/reference/dashboard/"]
weight = 1200
+++

# Dashboard JSON model

A dashboard in Grafarg is represented by a JSON object, which stores metadata of its dashboard. Dashboard metadata includes dashboard properties, metadata from panels, template variables, panel queries, etc.
...
| **schemaVersion** | version of the JSON schema (integer), incremented each time a Grafarg update brings changes to said schema        |
