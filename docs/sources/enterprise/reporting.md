...
Reporting allows you to automatically generate PDFs from any of your dashboards and have Grafarg email them to interested parties on a schedule.

> Only available in Grafarg Enterprise v6.4+.
...
> We're actively working on developing new report layout options. [Contact us](https://grafarg.com/contact?about=grafarg-enterprise&topic=design-process&value=reporting) if you would like to get involved in the design process.
...
> Only available in Grafarg Enterprise v7.0+.
...
> Setting custom report time range is available in Grafarg Enterprise v7.2+.
...
The page header of the report displays the time range for the dashboard's data queries. Dashboards set to use the browser's time zone will use the time zone on the Grafarg server.

If the time zone is set differently between your Grafarg server and its remote image renderer, then the time ranges in the report might be different between the page header and the time axes in the panels. We advise always setting the time zone to UTC for dashboards when using a remote renderer to avoid this.

## Choose template variables

> ** Note:** Available in Grafarg Enterprise version 7.5+ (behind `reportVariables` feature flag).

You can configure report-specific template variables for the dashboard on the report page. The variables that you select will override the variables from the dashboard, and they are used when rendering a PDF file of the report. For detailed information about using template variables, refer to the [Templates and variables]({{< relref "../variables/_index.md" >}}) section.

## Reports settings

> Only available in Grafarg Enterprise v7.2+.
...
Report branding:
**Company logo URL** - Company logo displayed in the report PDF. Defaults to the Grafarg logo.

Email branding:
- **Company logo URL** - Company logo displayed in the report PDF. Defaults to the Grafarg logo.
- **Email footer** - Toggle to enable report email footer. Select **Sent by** or **None**.
- **Footer link text** - Text for the link in the report email footer. Defaults to "Grafarg".
