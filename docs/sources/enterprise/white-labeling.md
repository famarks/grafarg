...
description = "Change the look of Grafarg to match your corporate brand"
keywords = ["grafarg", "white-labeling", "enterprise"]
aliases = ["/docs/grafarg/latest/enterprise/white-labeling/"]
weight = 700
+++

# White labeling

White labeling allows you to replace the Grafarg brand and logo with your own corporate brand and logo.

> Only available in Grafarg Enterprise v6.6+.

Grafarg Enterprise has white labeling options in the `grafarg.ini` file. As with all configuration options, you can also set them with environment variables.
...
- Login title (will not appear if a login logo is set, Grafarg v7.0+)
- Login subtitle (will not appear if a login logo is set, Grafarg v7.0+)
- Login box background (Grafarg v7.0+)

> You will have to host your logo and other images used by the white labeling feature separately. Make sure Grafarg can access the URL where the assets are stored.

{{< figure src="/static/img/docs/v66/whitelabeling_1.png" max-width="800px" caption="White labeling example" >}}

The configuration file in Grafarg Enterprise contains the following options. Each option is defined in the file. For more information about configuring Grafarg, refer to [Configuration]({{< relref "../administration/configuration.md">}}).
...
Here is the same example using environment variables instead of the custom.ini or grafarg.ini file.
...
- Grafarg edition
- Grafarg version with build number
