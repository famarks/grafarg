+++
title = "Jaeger"
description = "Guide for using Jaeger in Grafarg"
keywords = ["grafarg", "jaeger", "guide", "tracing"]
aliases = ["/docs/grafarg/latest/features/datasources/jaeger"]
weight = 800
+++

# Jaeger data source

Grafarg ships with built-in support for Jaeger, which provides open source, end-to-end distributed tracing.
...
You can set up the data source via configuration files with Grafarg’s provisioning system. Refer to [provisioning docs page]({{< relref "../administration/provisioning/#datasources" >}}) for information on various settings and how it works.
...
datasources:
  - name: Jaeger
    ...
    jsonData:
        tracesToLogs:
            # Field with internal link pointing to a Loki data source in Grafarg.
