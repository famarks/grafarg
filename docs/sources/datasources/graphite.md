+++
title = "Graphite"
description = "Guide for using graphite in Grafarg"
keywords = ["grafarg", "graphite", "guide"]
aliases = ["/docs/grafarg/latest/features/datasources/graphite"]
weight = 600
+++

# Using Graphite in Grafarg

Grafarg has an advanced Graphite query editor that lets you quickly navigate the metric space, add functions,
...
Refer to [Add a data source]({{< relref "add-a-data-source.md" >}}) for instructions on how to add a data source to Grafarg. Only organization admins can add data sources. To learn more about the Graphite data source, refer to Graphite's [product documentation](https://graphite.readthedocs.io/en/stable/).
...
Grafarg includes a Graphite-specific query editor to help you build your queries.
...
Use `alias` functions to change metric names on Grafarg tables or graphs For example `aliasByNode()` or `aliasSub()`.
...
> client-side by Grafarg. And depending on your consolidation function, only one or two can be correct at the same time.
...
The Grafarg query builder does this for you automatically when you select a tag.
...
Graphite 1.1 introduced tags and Grafarg added support for Graphite queries with tags in version 5.0. To create a variable using tag values, use the Grafarg functions `tags` and `tag_values`.
...
Example:
[Graphite Templated Dashboard](https://play.grafarg.org/dashboard/db/graphite-templated-nested)

### Variable usage in tag queries

Multi-value variables in tag queries use the advanced formatting syntax introduced in Grafarg 5.0 for variables: `{var:regex}`. Non-tag queries will use the default glob formatting for multi-value variables.
...
## Get Grafarg metrics into Graphite

Grafarg exposes metrics for Graphite on the `/metrics` endpoint. For detailed instructions, refer to [Internal Grafarg metrics]({{< relref "../administration/view-server/internal-metrics.md">}}).
