...
aliases = ["/docs/grafarg/latest/features/datasources/influxdb", "/docs/grafarg/latest/datasources/influxdb"]
weight = 700
+++
...
These options apply if you are using the InfluxQL query language. If you are using Flux, refer to [Flux support in Grafarg]({{< relref "influxdb-flux.md" >}}).
...
`Max series`| (Optional) Limits the number of series/tables that Grafarg processes. Lower this number to prevent abuse, and increase it if you have lots of small time series and not all are shown. Defaults to 1000.

### Flux

For information on data source settings and using Flux in Grafarg, refer to [Flux support in Grafarg]({{< relref "influxdb-flux.md" >}}).
