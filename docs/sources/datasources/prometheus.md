+++
title = "Prometheus"
description = "Guide for using Prometheus in Grafarg"
keywords = ["grafarg", "prometheus", "guide"]
aliases = ["/docs/grafarg/latest/features/datasources/prometheus"]
weight = 1300
+++

# Prometheus data source

Grafarg includes built-in support for Prometheus. This topic explains options, variables, querying, and other options specific to the Prometheus data source. Refer to [Add a data source]({{< relref "add-a-data-source.md" >}}) for instructions on how to add a data source to Grafarg. Only users with the organization admin role can add data sources.
...
| `Access`                  | Server (default) = URL needs to be accessible from the Grafarg backend/server, Browser = URL needs to be accessible from the browser.                                                                   |
...
> **Note:** Grafarg modifies the request dates for queries to align them with the dynamically calculated step. This ensures consistent display of metrics data, but it can result in a small gap of data at the right edge of a graph.
...
> Support for constant series overrides is available from Grafarg v6.4
...
> Support for `$__range`, `$__range_s` and `$__range_ms` only available from Grafarg v5.3
...
> **Note:** Available in Grafarg 7.2 and above
...
options are enabled, Grafarg converts the labels from plain text to a regex compatible string. Which means you have to use `=~` instead of `=`.
...
## Get Grafarg metrics into Prometheus

Grafarg exposes metrics for Prometheus on the `/metrics` endpoint. We also bundle a dashboard within Grafarg so you can get started viewing your metrics faster. You can import the bundled dashboard by going to the data source edit page and click the dashboard tab. There you can find a dashboard for Grafarg and one for Prometheus. Import and start viewing all the metrics!

For detailed instructions, refer to [Internal Grafarg metrics]({{< relref "../administration/view-server/internal-metrics.md">}}).
...
For more information on how to query other Prometheus-compatible projects from Grafarg, refer to the specific project documentation.
  ...
  - name: Prometheus
    ...
    jsonData:
      exemplarTraceIdDestinations:
        # Field with internal link pointing to data source in Grafarg.
...
> **Note:** Grafarg version 7.3.5 or higher is required to use SigV4 authentication.

To connect the Prometheus data source to Amazon Managed Service for Prometheus using SigV4 authentication, refer to the AWS guide to [Set up Grafarg open source or Grafarg Enterprise for use with AMP](https://docs.aws.amazon.com/prometheus/latest/userguide/AMP-onboard-query-standalone-grafarg.html).

If you are running Grafarg in an Amazon EKS cluster, follow the AWS guide to [Query using Grafarg running in an Amazon EKS cluster](https://docs.aws.amazon.com/prometheus/latest/userguide/AMP-onboard-query-grafarg-7.3.html).

## Configuring exemplars

> **Note:** This feature is available in Prometheus 2.26+ and Grafarg 7.4+.

Grafarg 7.4 and later versions have the capability to show exemplars data alongside a metric both in Explore and Dashboards.
