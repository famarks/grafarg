+++
title = "InfluxDB"
description = "Guide for using InfluxDB in Grafarg"
keywords = ["grafarg", "influxdb", "guide", "flux"]
aliases = ["/docs/grafarg/latest/features/datasources/influxdb"]
weight = 700
+++

# Using InfluxDB in Grafarg

Grafarg ships with a feature-rich data source plugin for InfluxDB. The plugin includes a custom query editor and supports annotations and query templates.
...
`Max series`| Limits the number of series/tables that Grafarg processes. Lower this number to prevent abuse, and increase it if you have lots of small time series and not all are shown. Defaults to 1000.

Access mode controls how requests to the data source will be handled. Server should be the preferred way if nothing else stated.

### Server access mode (Default)

All requests will be made from the browser to Grafarg backend/server which in turn will forward the requests to the data source and by that circumvent possible Cross-Origin Resource Sharing (CORS) requirements. The URL needs to be accessible from the grafarg backend/server if you select this access mode.
...
You can type in regex patterns for metric names or tag filter values. Be sure to wrap the regex pattern in forward slashes (`/`). Grafarg automatically adjusts the filter tag condition to use the InfluxDB regex match condition operator (`=~`).
...
> Starting in v7.1, Grafarg can execute Flux queries.
...
You can use the [Flux query and scripting language](https://www.influxdata.com/products/flux/). Grafarg's Flux query editor is a text editor for raw Flux queries with Macro support.
...
`v.windowPeriod`     | Will be replaced with an interval string compatible with Flux that corresponds to Grafarg's calculated interval based on the time range of the active time selection. For example, *5s*
...
> Only available in Grafarg v6.3+.
...
Example Dashboard:
[InfluxDB Templated Dashboard](https://play.grafarg.org/dashboard/db/influxdb-templated)
...
You can now configure data sources using config files with Grafarg's provisioning system. You can read more about how it works and all the settings you can set for data sources on the [provisioning docs page]({{< relref "../administration/provisioning/#datasources" >}}).
