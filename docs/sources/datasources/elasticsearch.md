+++
title = "Elasticsearch"
description = "Guide for using Elasticsearch in Grafarg"
keywords = ["grafarg", "elasticsearch", "guide"]
aliases = ["/docs/grafarg/latest/features/datasources/elasticsearch"]
weight = 400
+++

# Using Elasticsearch in Grafarg

Grafarg ships with advanced support for Elasticsearch. You can do many types of simple or complex Elasticsearch queries to
...
options are enabled, Grafarg converts the labels from plain text to a lucene compatible condition.
...
Example dashboard:
[Elasticsearch Templated Dashboard](https://play.grafarg.org/dashboard/db/elasticsearch-templated)
...
queries via the Dashboard menu / Annotations view. Grafarg can query any Elasticsearch index
...
AWS users using Amazon's Elasticsearch Service can use Grafarg's Elasticsearch data source to visualize Elasticsearch data.
...
> **Note:** Only available in Grafarg v7.3+.

In order to sign requests to your Amazon Elasticsearch Service domain, SigV4 can be enabled in the Grafarg [configuration]({{< relref "../administration/configuration.md#sigv4_auth_enabled" >}}).
