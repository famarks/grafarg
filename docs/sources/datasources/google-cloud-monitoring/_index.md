...
description = "Guide for using Google Cloud Monitoring in Grafarg"
keywords = ["grafarg", "stackdriver", "google", "guide", "cloud", "monitoring"]
aliases = ["/docs/grafarg/latest/features/datasources/stackdriver", "/docs/grafarg/latest/datasources/cloudmonitoring/", "/docs/grafarg/latest/features/datasources/cloudmonitoring/"]
weight = 200
+++

# Using Google Cloud Monitoring in Grafarg

Grafarg ships with built-in support for Google Cloud Monitoring. Add it as a data source to build dashboards for your Google Cloud Monitoring metrics. For instructions on how to add a data source, refer to [Add a data source]({{< relref "../add-a-data-source.md" >}}). Only users with the organization admin role can add data sources.

> **Note** Before Grafarg v7.1, Google Cloud Monitoring was referred to as Google Stackdriver.
...
There are two ways to authenticate the Google Cloud Monitoring plugin - either by uploading a Google JWT file, or by automatically retrieving credentials from Google metadata server. The latter option is only available when running Grafarg on GCE virtual machine.

### Using a Google Service Account Key File

To authenticate with the Google Cloud Monitoring API, you need to create a Google Cloud Platform (GCP) Service Account for the Project you want to show data for. A Grafarg data source integrates with one GCP Project. If you want to visualize data from multiple GCP Projects then you need to create one data source per GCP Project.
...
1. Upload it to Grafarg on the data source Configuration page. You can either upload the file or paste in the contents of the file.

   {{< figure src="/static/img/docs/v71/cloudmonitoring_grafarg_upload_key.png" max-width="550px" class="docs-image--no-shadow" caption="Upload service key file to Grafarg" >}}

1. The file contents will be encrypted and saved in the Grafarg database. Don't forget to save after uploading the file!

   {{< figure src="/static/img/docs/v71/cloudmonitoring_grafarg_key_uploaded.png" max-width="600px" class="docs-image--no-shadow" caption="Service key file is uploaded to Grafarg" >}}

### Using GCE Default Service Account

If Grafarg is running on a Google Compute Engine (GCE) virtual machine, it is possible for Grafarg to automatically retrieve default credentials from the metadata server. This has the advantage of not needing to generate a private key file for the service account and also not having to upload the file to Grafarg. However for this to work, there are a few preconditions that need to be met.
...
Google Cloud Monitoring supports different kinds of metrics like `GAUGE`, `DELTA,` and `CUMULATIVE`. They support different aggregation options, for example, reducers and aligners. The Grafarg query editor displays the list of available aggregation methods for a selected metric and sets a default reducer and aligner when you select the metric.

#### Filter

To add a filter, click the plus icon and choose a field to filter by and enter a filter value e.g. `instance_name = grafarg-1`. You can remove the filter by clicking on the filter name and select `--remove filter--`.
...
The `Alignment Period` groups a metric by time if an aggregation is chosen. The default is to use the GCP Google Cloud Monitoring default groupings (which allows you to compare graphs in Grafarg with graphs in the Google Cloud Monitoring UI).
...
The other automatic option is `grafarg auto`. This will automatically set the group by time depending on the time range chosen and the width of the time series panel. For more information about grafarg auto, refer to the [interval variable](http://docs.grafarg.org/variables/templates-and-variables/#the-interval-variable).
...
| -------------------------------- | ---------------------------------------- | --------------------------------- | ---------------- |
| `{{metric.label.xxx}}`           | returns the metric label value           | `{{metric.label.instance_name}}`  | `grafarg-1-prod` |
| `{{resource.label.xxx}}`         | returns the resource label value         | `{{resource.label.zone}}`         | `us-east1-b`     |
| `{{metadata.system_labels.xxx}}` | returns the meta data system label value | `{{metadata.system_labels.name}}` | `grafarg`        |
...
#### Deep linking from Grafarg panels to the Metrics Explorer in Google Cloud Console

> **Note:** Available in Grafarg v7.1 and later versions.
...
Click on a time series in the panel to see a context menu with a link to View in Metrics Explorer in Google Cloud Console. Clicking that link opens the Metrics Explorer in the Google Cloud Console and runs the query from the Grafarg panel there.
...
Grafarg issues one query to the Cloud Monitoring API per query editor row, and each API response includes a unit. Grafarg will attempt to convert the returned unit into a unit that is understood by the Grafarg time series panel. If the conversion was successful, then the unit will be displayed on the Y-axis on the panel. If the query editor rows returned different units, then the unit from the last query editor row is used in the time series panel.

### SLO (Service Level Objective) queries

> **Note:** Available in Grafarg v7.0 and later versions.
...
The friendly names for the time series selectors are shown in Grafarg. Here is the mapping from the friendly name to the system name that is used in the Service Monitoring documentation:
...
| `Alignment periods`              | Returns a list of all alignment periods that are available in Google Cloud Monitoring query editor in Grafarg |
...
Why two ways? The first syntax is easier to read and write but does not allow you to use a variable in the middle of a word. When the _Multi-value_ or _Include all value_ options are enabled, Grafarg converts the labels from plain text to a regex compatible string, which means you have to use `=~` instead of `=`.
...
You can configure data sources using config files with Grafarg's provisioning system. Read more about how it works and all the settings you can set for data sources on the [provisioning docs page]({{< relref "../../administration/provisioning/#datasources" >}})
