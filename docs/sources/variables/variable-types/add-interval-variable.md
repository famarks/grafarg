...
aliases = ["/docs/grafarg/latest/variables/add-interval-variable.md"]
weight = 600
+++
...
1. (optional) Turn on the **Auto Option** if you want to add the `auto` option to the list. This option allows you to specify how many times the current time range should be divided to calculate the current `auto` time span. If you turn it on, then two more options appear:
   ...
   - **Min Interval -** The minimum threshold below which the step count intervals will not divide the time. To continue the 30 minute example, if the minimum interval is set to 2m, then Grafarg would group the data into 15 two-minute increments.
...
A more complex Graphite example, from the [Graphite Template Nested Requests panel](https://play.grafarg.org/d/000000056/graphite-templated-nested?editPanel=2&orgId=1):
