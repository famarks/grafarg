...
```bash
http://<my_grafarg_server_name_or_ip>:<grafarg_server_port>/login/github
```

This callback URL must match the full HTTP address that you use in your
browser to access Grafarg, but with the suffix path of `/login/github`.
When the GitHub OAuth application is created you will get a Client ID and a
Client Secret. Specify these in the Grafarg configuration file. For
example:

## Enable GitHub in Grafarg
...
Restart the Grafarg back-end. You should now see a GitHub login button
...
your Grafarg instance. For example
...
With Team Sync you can map your GitHub org teams to teams in Grafarg so that your users will automatically be added to
the correct teams.
