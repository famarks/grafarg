...
description = "Grafarg LDAP Authentication Guide "
keywords = ["grafarg", "configuration", "documentation", "ldap", "active directory"]
aliases = ["/docs/grafarg/latest/installation/ldap/"]
weight = 300
+++

# LDAP Authentication

The LDAP integration in Grafarg allows your Grafarg users to login with their LDAP credentials. You can also specify mappings between LDAP
group memberships and Grafarg Organization user roles.

> [Enhanced LDAP authentication]({{< relref "../enterprise/enhanced_ldap.md" >}}) is available in [Grafarg Enterprise]({{< relref "../enterprise" >}}).

## Supported LDAP Servers

Grafarg uses a [third-party LDAP library](https://github.com/go-ldap/ldap) under the hood that supports basic LDAP v3 functionality.
...
specific configuration file (default: `/etc/grafarg/ldap.toml`).
...
# Path to the LDAP specific configuration file (default: `/etc/grafarg/ldap.toml`)
config_file = /etc/grafarg/ldap.toml

# Allow sign up should almost always be true (default) to allow new Grafarg users to be created (if LDAP authentication is ok). If set to
# false only pre-existing Grafarg users will be able to login (if LDAP authentication is ok).
allow_sign_up = true
```

## Grafarg LDAP Configuration

Depending on which LDAP server you're using and how that's configured your Grafarg LDAP configuration may vary.
...
search_base_dns = ["dc=grafarg,dc=org"]

# group_search_filter = "(&(objectClass=posixGroup)(memberUid=%s))"
# group_search_filter_user_attribute = "distinguishedName"
# group_search_base_dns = ["ou=groups,dc=grafarg,dc=org"]
...
> Only available in Grafarg v6.4+

Grafarg has an LDAP debug view built-in which allows you to test your LDAP configuration directly within Grafarg. At the moment of writing, only Grafarg admins can use the LDAP debug view.
...
When the user DN is found a second bind is performed with the user provided username and password (in the normal Grafarg login form).
...
```bash
bind_dn = "cn=%s,o=users,dc=grafarg,dc=org"
```

In this case you skip providing a `bind_password` and instead provide a `bind_dn` value with a `%s` somewhere. This will be replaced with the username entered in on the Grafarg login page.
...
In `[[servers.group_mappings]]` you can map an LDAP group to a Grafarg organization and role.  These will be synced every time the user logs in, with LDAP being
the authoritative source. So, if you change a user's role in the Grafarg Org. Users page, this change will be reset the next time the user logs in. If you
...
group_dn = "cn=superadmins,dc=grafarg,dc=org"
org_role = "Admin"
grafarg_admin = true # Available in Grafarg v5.3 and above

[[servers.group_mappings]]
group_dn = "cn=admins,dc=grafarg,dc=org"
org_role = "Admin"

[[servers.group_mappings]]
group_dn = "cn=users,dc=grafarg,dc=org"
org_role = "Editor"
...
`org_id` | No | The Grafarg organization database id. Setting this allows for multiple group_dn's to be assigned to the same `org_role` provided the `org_id` differs | `1` (default org id)
`grafarg_admin` | No | When `true` makes user of `group_dn` Grafarg server admin. A Grafarg server admin has admin access over all organizations and users. Available in Grafarg v5.3 and above | `false`
...
Grafarg does support receiving information from multiple LDAP servers.
