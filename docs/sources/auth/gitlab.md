...
```
https://grafarg.example.com/login/gitlab
```

where `https://grafarg.example.com` is the URL you use to connect to Grafarg.
Adjust it as needed if you don't use HTTPS or if you use a different port; for
instance, if you access Grafarg at `http://203.0.113.31:3000`, you should use
...
## Enable GitLab in Grafarg

Add the following to your Grafarg configuration file to enable GitLab
authentication:
...
Restart the Grafarg backend for your changes to take effect.
...
who can authenticate on GitLab will be able to login on your Grafarg instance;
if you use the public `gitlab.com`, it means anyone in the world would be able
to login on your Grafarg instance.
...
With Team Sync you can map your GitLab groups to teams in Grafarg so that your users will automatically be added to
the correct teams.
