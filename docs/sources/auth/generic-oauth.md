...
You can configure many different OAuth2 authentication services with Grafarg using the generic OAuth2 feature. Examples:
...
This callback URL must match the full HTTP address that you use in your browser to access Grafarg, but with the suffixed path of `/login/generic_oauth`.

You may have to set the `root_url` option of `[server]` for the callback URL to be
correct. For example in case you are serving Grafarg behind a proxy.
...
Grafarg will attempt to determine the user's e-mail address by querying the OAuth provider as described below in the following order until an e-mail address is found:
...
**Note**: Only available in Grafarg v6.4+.
1. Check for the presence of an e-mail address in the `attributes` map encoded in the OAuth `id_token` parameter. By default Grafarg will perform a lookup into the attributes map using the `email:primary` key, however, this is configurable and can be adjusted by using the `email_attribute_name` configuration option.
...
> **Note:** `name_attribute_path` is available in Grafarg 7.4+.
...
1. Go to the Settings tab and set:
    - Allowed Callback URLs: `https://<grafarg domain>/login/generic_oauth`

1. Click Save Changes, then use the values at the top of the page to configure Grafarg:
...
1. Create a memorable unique Application ID, e.g. "grafarg", "grafarg_aws", etc.
...
1. Put the URL to the front page of your Grafarg instance into the "Resource Application URL" field.

1. Add an authorized Redirect URI like https://your-grafarg-server/login/generic_oauth

1. Set up permissions, policies, etc. just like any other Centrify app

1. Configure Grafarg as follows:
...
1. Create a new Custom Connector with the following settings:
    ...
    - Redirect URI: `https://<grafarg domain>/login/generic_oauth`
    - Signing Algorithm: RS256
    - Login URL: `https://<grafarg domain>/login/generic_oauth`

    then:
1. Add an App to the Grafarg Connector:
    - Display Name: Grafarg

    then:
1. Under the SSO tab on the Grafarg App details page you'll find the Client ID and Client Secret.

    Your OneLogin Domain will match the URL you use to access OneLogin.

    Configure Grafarg as follows:
...
In the following example user will get `Editor` as role when authenticating. The value of the property `role` will be the resulting role if the role is a proper Grafarg role, i.e. `Viewer`, `Editor` or `Admin`.
