...
An _organization_ is a group of users on a Grafarg server. Each user can belong to more than one organization. Every member of the organization has a _role_ in that organization that grants them a certain level of permissions. For more information, refer to [Organization roles]({{< relref "../../permissions/organization_roles.md" >}}).
...
See a complete list of users with accounts in your Grafarg organization. If necessary, you can use the search field to filter the list.
...
Organization Admins can invite users to their Grafarg organizations and manage invitations. When an invited user signs in to Grafarg, a user account is created for them if one does not already exist.
...
1. Enter the following information:
   - **Email or Username -** Either the email or username that the user will use to sign in to Grafarg.
   ...
   - **Send invite email**
     - **Yes -** If your organization has SMTP set up, then Grafarg sends an email to the user inviting them to log in to Grafarg and join your organization.
     - **No -** The user is not sent an invitation, but they can sign in to the Grafarg server with the email or username that you entered.
1. Click **Submit**.
...
Grafarg displays a list of pending invitations. If necessary, you can use the search field to filter the list.
...
1. Click the **Role** list in the user account that you want to change. Grafarg displays the list of available roles.
