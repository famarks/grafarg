...
Grafarg offers several options for grouping users. Each level has different tools for managing user accounts and different tasks that they can perform.
...
The highest and broadest level of user group in Grafarg is the server. Every user with an account in a Grafarg instance is a member of the server group.

Grafarg Server Admins are user accounts that have the Grafarg Admin option set to **Yes**. They can manage individual user accounts and organizations on their server.
...
Teams are mostly managed by Organization Admins. However, if the Grafarg server setting [editors_can_admin]({{< relref "../administration/configuration.md#editors_can_admin" >}}) is applied, then users who are assigned the Team Admin role can also manage teams in their organization and users assigned to their teams.

Team Admins can [Manage teams]({{< relref "manage-teams/index.md" >}}).

## Users

Users are named accounts in Grafarg with granted permissions to access resources throughout Grafarg. All users can manage their own accounts to a limited extent.
...
Set up users and teams in our tutorial on how to [Create users and teams](https://grafarg.com/tutorials/create-users-and-teams).
