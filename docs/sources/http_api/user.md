+++
title = "User HTTP API "
description = "Grafarg User HTTP API"
keywords = ["grafarg", "http", "documentation", "api", "user"]
aliases = ["/docs/grafarg/latest/http_api/user/"]
+++
...
Default value for the `perpage` parameter is `1000` and for the `page` parameter is `1`. Requires basic authentication and that the authenticated user is a Grafarg Admin.
...
Requires basic authentication and that the authenticated user is a Grafarg Admin.
...
curl -X PUT -H "Content-Type: application/json" -d '{
  ...
  "newPassword": "newpass",
  "confirmNew": "newpass"
}' http://admin:oldpass@<your_grafarg_host>:3000/api/user/password
```
...
Switch user context to the given organization. Requires basic authentication and that the authenticated user is a Grafarg Admin.
