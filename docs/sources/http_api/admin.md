...
description = "Grafarg Admin HTTP API"
keywords = ["grafarg", "http", "documentation", "api", "admin"]
aliases = ["/docs/grafarg/latest/http_api/admin/"]
+++
...
the permission of server admin, only users can be given that permission. So in order to use these API calls you will have to use Basic Auth and the Grafarg user
must have the Grafarg Admin permission. (The default admin user is called `admin` and has permission to use this API.)

## Settings

`GET /api/admin/settings`

Only works with Basic Authentication (username and password). See [introduction](http://docs.grafarg.org/http_api/admin/#admin-api) for an explanation.
...

{
  ...
  "paths":{
    "data":"/tsdb/grafarg",
    "logs":"/logs/apps/grafarg"},
    "security":{
    "admin_password":"************",
    "admin_user":"admin",
    "cookie_remember_name":"grafarg_remember",
    "cookie_username":"grafarg_user",
  ...
  "smtp":{
    "cert_file":"",
    "enabled":"false",
    "from_address":"admin@grafarg.localhost",
    "from_name":"Grafarg",
...
Create new user. Only works with Basic Authentication (username and password). See [introduction](http://docs.grafarg.org/http_api/admin/#admin-api) for an explanation.
