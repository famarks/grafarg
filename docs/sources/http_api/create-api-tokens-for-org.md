...
keywords = ["grafarg", "tutorials", "API", "Token", "Org", "Organization"]
aliases =["/docs/grafarg/latest/tutorials/api_org_token_howto/"]
weight = 150
+++

# Create API tokens and dashboards for an organization

Use the Grafarg API to set up new Grafarg organizations or to add dynamically generated dashboards to an existing organization.

## Authentication

There are two authentication methods to access the API:

- Basic authentication: A Grafarg Admin user can access some parts of the Grafarg API through basic authentication.
...
1. [Create the org](http://docs.grafarg.org/http_api/org/#create-organization). Here is an example using curl:
    ```bash
...
1. Optional step. If the org was created previously and/or step 3 fails then first [add your Admin user to the org](http://docs.grafarg.org/http_api/org/#add-user-in-organization):
    ```bash
...
1. [Switch the org context for the Admin user to the new org](http://docs.grafarg.org/http_api/user/#switch-user-context-for-signed-in-user):
    ```bash
    curl -X POST http://admin:admin@localhost:3000/api/user/using/<id of new org>
    ```

1. [Create the API token](http://docs.grafarg.org/http_api/auth/#create-api-key):
    ```bash
...
1. [Add a dashboard](http://docs.grafarg.org/http_api/dashboard/#create-update-dashboard) using the key (or bearer token as it is also called):
  ...
  > **Note:** If you export a dashboard for sharing externally using the Share > Export menu in the Grafarg UI, you cannot import that dashboard. Instead, click **View JSON** and save it to a file or fetch the JSON output through the API.
