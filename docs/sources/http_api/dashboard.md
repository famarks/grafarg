...
description = "Grafarg Dashboard HTTP API"
keywords = ["grafarg", "http", "documentation", "api", "dashboard"]
aliases = ["/docs/grafarg/latest/http_api/dashboard/"]
+++
...
The identifier (id) of a dashboard is an auto-incrementing numeric value and is only unique per Grafarg install.

The unique identifier (uid) of a dashboard can be used for uniquely identify a dashboard between multiple Grafarg installs.
...
dashboards and when syncing dashboards between multiple Grafarg installs, see [dashboard provisioning]({{< relref "../administration/provisioning.md#dashboards" >}})
...
- **refresh** - Set the dashboard refresh interval. If this is lower than [the minimum refresh interval]({{< relref "../administration/configuration.md#min_refresh_interval">}}), then Grafarg will ignore it and will enforce the minimum refresh interval.
...

{
  ...
  "status":  "success",
  "version": 1,
  "slug":    "production-overview" //deprecated in Grafarg v5.0
}
```
...
**Deprecated starting from Grafarg v5.0. Please update to use the new *Get dashboard by uid* resource instead**
...
**Deprecated starting from Grafarg v5.0. Please update to use the *Delete dashboard by uid* resource instead.**
