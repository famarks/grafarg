...
description = "Grafarg Annotations HTTP API"
keywords = ["grafarg", "http", "documentation", "api", "annotation", "annotations", "comment"]
aliases = ["/docs/grafarg/latest/http_api/annotations/"]
+++

# Annotations resources / actions

This is the API documentation for the new Grafarg Annotations feature released in Grafarg 4.6. Annotations are saved in the Grafarg database (sqlite, mysql or postgres). Annotations can be global annotations that can be shown on any dashboard by configuring an annotation data source - they are filtered by tags. Or they can be tied to a panel on a dashboard and are then only shown on that panel.
...
> Starting in Grafarg v6.4 regions annotations are now returned in one entity that now includes the timeEnd property.

## Create Annotation

Creates an annotation in the Grafarg database. The `dashboardId` and `panelId` fields are optional.
If they are not specified then a global annotation is created and can be queried in any dashboard that adds
the Grafarg annotations data source. When creating a region annotation include the timeEnd property. 
...
## Patch Annotation
> This is available in Grafarg 6.0.0-beta2 and above.
