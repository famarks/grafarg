...
description = "Grafarg Folder HTTP API"
keywords = ["grafarg", "http", "documentation", "api", "folder"]
aliases = ["/docs/grafarg/latest/http_api/folder/"]
+++
...
The identifier (id) of a folder is an auto-incrementing numeric value and is only unique per Grafarg install.

The unique identifier (uid) of a folder can be used for uniquely identify folders between multiple Grafarg installs. It's automatically generated if not provided when creating a folder. The uid allows having consistent URLs for accessing folders and when syncing folders between multiple Grafarg installs. This means that changing the title of a folder will not break any bookmarked links to that folder.
