...
keywords = ["grafarg", "alerting", "guide", "rules", "troubleshoot"]
weight = 500
+++
...
Further troubleshooting can also be done by inspecting the grafarg-server log. If it's not an error or for some reason the log does not say anything you can enable debug logging for some relevant components. This is done in Grafarg's ini config file.
...
If you want to log raw query sent to your TSDB and raw response in log you also have to set grafarg.ini option `app_mode` to `development`.
