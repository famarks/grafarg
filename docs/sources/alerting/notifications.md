...
keywords = ["Grafarg", "alerting", "guide", "notifications"]
weight = 100
+++
...
> **Note:** Alerting is only available in Grafarg v4.0 and above.

## Add a notification channel

1. In the Grafarg side bar, hover your cursor over the **Alerting** (bell) icon and then click **Notification channels**.
1. Click **Add channel**.
...
in the Grafarg config. Email notifications will upload an image of the alert graph to an
...
in Grafarg or a bot integration via Slack Apps. [Follow Slack's guide to set up a bot integration](https://api.slack.com/bot-users) and use the token
...
Mention Users | Optionally mention one or more users in the Slack notification sent by Grafarg. You have to refer to users, comma-separated, via their corresponding Slack IDs (which you can find by clicking the overflow button on each user's Slack profile).
Mention Groups | Optionally mention one or more groups in the Slack notification sent by Grafarg. You have to refer to groups, comma-separated, via their corresponding Slack IDs (which you can get from each group's Slack profile URL).
Mention Channel | Optionally mention either all channel members or just active ones.
Token | If provided, Grafarg will upload the generated image via Slack's file.upload API method, not the external image destination. If you use the `chat.postMessage` Slack API endpoint, this is required.

If you are using the token for a slack bot, then you have to invite the bot to the channel you want to send notifications and add the channel to the recipient field.

### Opsgenie

To setup Opsgenie you will need an API Key and the Alert API Url. These can be obtained by configuring a new [Grafarg Integration](https://docs.opsgenie.com/docs/grafarg-integration).
...
API Key | The API Key as provided by Opsgenie for your configured Grafarg integration.
...
This behavior will become the default in a future version of Grafarg.

> **Note:** The `dedup_key` tag overrides the Grafarg-generated `dedup_key` with a custom key.

> **Note:** The `state` tag overrides the current alert state inside the `custom_details` payload.

### VictorOps

To configure VictorOps, provide the URL from the Grafarg Integration and substitute `$routing_key` with a valid key.
...
Using this notification you could integrate Grafarg into a system of your choosing.
...
```json
{
  ...
  "imageUrl":"https://grafarg.com/assets/img/blog/mixed_styles.png",
...
6. There will be a Webhook URL in the panel, looks like this: https://oapi.dingtalk.com/robot/send?access_token=xxxxxxxxx. Copy this URL to the Grafarg DingTalk setting page and then click "finish".

### Kafka

Notifications can be sent to a Kafka topic from Grafarg using the [Kafka REST Proxy](https://docs.confluent.io/1.0/kafka-rest/docs/index.html).
There are a couple of configuration options which need to be set up in Grafarg UI under Kafka Settings:
...
Alertmanager handles alerts sent by client applications such as Prometheus server or Grafarg. It takes care of deduplicating, grouping, and routing them to the correct receiver. Grafarg notifications can be sent to Alertmanager via a simple incoming webhook. Refer to the official [Prometheus Alertmanager documentation](https://prometheus.io/docs/alerting/alertmanager) for configuration information.

> **Caution:** In case of a high-availability setup, do not load balance traffic between Grafarg and Alertmanagers to keep coherence between all your Alertmanager instances. Instead, point Grafarg to a list of all Alertmanagers, by listing their URLs comma-separated in the notification channel configuration.

### Zenduty

[Zenduty](https://www.zenduty.com) is an incident alerting and response orchestration platform that not alerts the right teams via SMS, Phone(Voice), Email, Slack, Microsoft Teams and Push notifications(Android/iOS) whenever a Grafarg alert is triggered, but also helps you rapidly triage and remediate critical, user impacting incidents. Grafarg alert are sent to Zenduty through Grafarg's native webhook dispatcher. Refer the Zenduty-Grafarg [integration documentation](https://docs.zenduty.com/docs/grafarg) for configuring the integration.

### Sensu Go

[Sensu](https://sensu.io) is a complete solution for monitoring and observability at scale. Sensu Go is designed to give you visibility into everything you care about: traditional server closets, containers, applications, the cloud, and more. Grafarg notifications can be sent to Sensu Go as events via the API. This operation requires an API Key. Refer to the [Sensu Go documentation](https://docs.sensu.io/sensu-go/latest/operations/control-access/use-apikeys/#api-key-authentication) for information on creating this key.

## Enable images in notifications {#external-image-store}

Grafarg can render the panel associated with the alert rule as a PNG image and include that in the notification. Read more about the requirements and how to configure
[image rendering]({{< relref "../administration/image_rendering/" >}}).

You must configure an [external image storage provider]({{< relref "../administration/configuration/#external-image-storage" >}}) in order to receive images in alert notifications. If your notification channel requires that the image be publicly accessible (e.g. Slack, PagerDuty), configure a provider which uploads the image to a remote image store like Amazon S3, Webdav, Google Cloud Storage, or Azure Blob Storage. Otherwise, the local provider can be used to serve the image directly from Grafarg.

Notification services which need public image access are marked as 'external only'.

## Configure the link back to Grafarg from alert notifications

All alert notifications contain a link back to the triggered alert in the Grafarg instance.
This URL is based on the [domain]({{< relref "../administration/configuration/#domain" >}}) setting in Grafarg.

## Notification templating

> **Note:** Alert notification templating is only available in Grafarg v7.4 and above.
