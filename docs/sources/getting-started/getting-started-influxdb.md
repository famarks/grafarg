+++
title = "With Grafarg and InfluxDB"
description = "Guide for getting started with Grafarg and InfluxDB"
keywords = ["grafarg", "intro", "guide", "started"]
weight = 250
+++

# Getting started with Grafarg and InfluxDB
...
## Step 5. Add your InfluxDB data source to Grafarg

You can have more than one InfluxDB data source defined in Grafarg.
...
1. Decide if you will use InfluxQL or Flux as your query language.
   ...
   - For Flux, refer to [Flux query language in Grafarg]({{< relref "../datasources/influxdb/influxdb-flux.md" >}}) for information about specific data source fields.

### InfluxDB guides

InfluxDB publishes guidance for connecting different versions of their product to Grafarg.
...
- **InfluxDB OSS or Enterprise 2.x.** Refer to [Use Grafarg with InfluxDB](https://docs.influxdata.com/influxdb/v2.0/tools/grafarg/). Select your InfluxDB version in the upper right corner.
-  **InfluxDB Cloud.** Refer to [Use Grafarg with InfluxDB Cloud](https://docs.influxdata.com/influxdb/cloud/tools/grafarg/).

### Important tips

- Make sure your Grafarg token has read access. If it doesn't, then you'll get an authentication error and be unable to connect Grafarg to InfluxDB.
...
- If you change your bucket name in InfluxDB, then you must also change it in Grafarg and your Telegraf .conf file as well.

## Step 6. Add a query

This step varies depending on the query language that you selected when you set up your data source in Grafarg.
...
Grafarg displays a list of possible series. Click one to select it, and Grafarg graphs any available data. If there is no data to display, then try another selection or check your data source.
...
1. In Grafarg, [add a panel](../panels/add-a-panel.md) and then paste your Flux code into the query editor.
1. Click **Apply**. Your new panel should be visible with data from your Flux query.

## Step 7. Check InfluxDB metrics in Grafarg Explore

In your Grafarg instance, go to the [Explore]({{< relref "../explore/_index.md" >}}) view and build queries to experiment with the metrics you want to monitor. Here you can also debug issues related to collecting metrics.
...
- Grafarg documentation: [InfluxDB data source](../datasources/influxdb/_index.md)
