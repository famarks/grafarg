...
aliases = ["/docs/grafarg/latest/administration/change-home-dashboard/"]
weight = 300
+++
...
The home dashboard is the first dashboard a user sees when they sign in to Grafarg. You can also navigate to the home dashboard manually.
...
Users with the Grafarg Server Admin flag on their account or access to the configuration file can define a JSON file to use as the home dashboard for all users on the server.
...
1. In the Export tab, click **Save to file**. Grafarg converts the dashboard to a JSON file and saves it locally.

### Use a JSON file as the home dashboard

1. Save your JSON file somewhere that Grafarg can access it. For example, in the Grafarg `data` folder of Grafarg.
1. Update your configuration file to set the path to the JSON file. Refer to [default_home_dashboard_path]({{< relref "../configuration.md">}}) for more information about modifying the Grafarg configuration files.
