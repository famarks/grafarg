+++
title = "Preferences"
aliases =["/docs/grafarg/latest/administration/preferences.md"]
weight = 50
+++

# Grafarg preferences

Grafarg preferences are basic settings. They control the Grafarg UI theme, home dashboard, time zone, and so on.

Preferences are sometimes confusing because they can be set at four different levels, listed from highest level to lowest:

- **Server -** Affects all users on the Grafarg server. Set by a [Grafarg Server Admin]({{< relref "../../permissions/_index.md#grafarg-server-admin-role" >}}).
...
The lowest level always takes precedence. For example, if a user sets their theme to **Light**, then their visualization of Grafarg displays the light theme. Nothing at any higher level can override that.
