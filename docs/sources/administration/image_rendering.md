...
description = ""
keywords = ["grafarg", "image", "rendering", "plugin"]
weight = 300
+++

# Image rendering

Grafarg supports automatic rendering of panels as PNG images. This allows Grafarg to automatically generate images of your panels to include in [alert notifications]({{< relref "../alerting/notifications.md" >}}).

>**Note:** Image rendering of dashboards is not supported at this time.

While an image is being rendered, the PNG image is temporarily written to the file system. When the image is rendered, the PNG image is temporarily written to the `png` folder in the Grafarg `data` folder.
...
## Install Grafarg Image Renderer plugin

The [Grafarg image renderer plugin](https://grafarg.com/grafarg/plugins/grafarg-image-renderer) is a plugin that runs on the backend and handles rendering panels and dashboards as PNG images using headless Chrome.

To install the plugin, refer to the [Grafarg Image Renderer Installation instructions](https://grafarg.com/grafarg/plugins/grafarg-image-renderer/?tab=installation).

## Run in custom Grafarg Docker image
...
If you still want to install the plugin in the Grafarg Docker image, refer to [Build with Grafarg Image Renderer plugin pre-installed]({{< relref "../installation/docker/#build-with-grafarg-image-renderer-plugin-pre-installed" >}}).

## Remote rendering service

> Requires an internet connection.

The [Grafarg Image Renderer plugin](https://grafarg.com/grafarg/plugins/grafarg-image-renderer) can also be run as a remote HTTP rendering service. In this setup, Grafarg renders an image by making a HTTP request to the remote rendering service, which in turn renders the image and returns it back in the HTTP response to Grafarg.

You can run the remote HTTP rendering service using Docker or as a standalone Node.js application.

### Run in Docker

The following example shows how to run Grafarg and the remote HTTP rendering service in two separate Docker containers using Docker Compose.
...
The following example describes how to build and run the remote HTTP rendering service as a standalone Node.js application and configure Grafarg appropriately.

1. Clone the [Grafarg image renderer plugin](https://grafarg.com/grafarg/plugins/grafarg-image-renderer) Git repository.
...
1. Update Grafarg configuration:
...
> Starting from Grafarg v7.0.0, all PhantomJS support has been removed. Please use the Grafarg Image Renderer plugin or remote rendering service.

## Troubleshoot image rendering

Enable debug log messages for rendering in the Grafarg configuration file and inspect the Grafarg server log.
...
### Grafarg image renderer plugin and remote rendering service
...
```bash
Rendering failed: Error: Failed to launch chrome!/var/lib/grafarg/plugins/grafarg-image-renderer/chrome-linux/chrome:
...
```bash
cd <grafarg-image-render plugin directory>
...
In many cases, Grafarg runs on internal servers and uses certificates that have not been signed by a CA ([Certificate Authority](https://en.wikipedia.org/wiki/Certificate_authority)) known to Chrome, and therefore cannot be validated. Chrome internally uses NSS ([Network Security Services](https://en.wikipedia.org/wiki/Network_Security_Services)) for cryptographic operations such as the validation of certificates.

If you are using the Grafarg Image Renderer with a Grafarg server that uses a certificate signed by such a custom CA (for example a company-internal CA), rendering images will fail and you will see messages like this in the Grafarg log:
...
t=2019-12-04T12:39:22+0000 lvl=error msg="Request Completed" logger=context userId=1 orgId=1 uname=admin method=GET path=/render/d-solo/zxDJxNaZk/graphite-metrics status=500 remote_addr=192.168.106.101 time_ms=310 size=1722 referer="https://grafarg.xxx-xxx/d/zxDJxNaZk/graphite-metrics?orgId=1&refresh=1m"
```

(The severity-level `error` in the above messages might be misspelled with a single `r`)

If this happens, then you have to add the certificate to the trust store. If you have the certificate file for the internal root CA in the file `internal-root-ca.crt.pem`, then use these commands to create a user specific NSS trust store for the Grafarg user (`grafarg` for the purpose of this example) and execute the following steps:

```[root@server ~]# [ -d /usr/share/grafarg/.pki/nssdb ] || mkdir -p /usr/share/grafarg/.pki/nssdb
[root@merver ~]# certutil -d sql:/usr/share/grafarg/.pki/nssdb -A -n internal-root-ca -t C -i /etc/pki/tls/certs/internal-root-ca.crt.pem
[root@server ~]# chown -R grafarg: /usr/share/grafarg/.pki/nssdb
```
...
installed on your system, then you can configure [Grafarg Image renderer plugin](#grafarg-image-renderer-plugin) to use this
instead of the pre-packaged version of Chromium.

> Please note that this is not recommended, since you may encounter problems if the installed version of Chrome/Chromium is not
> compatible with the [Grafarg Image renderer plugin](#grafarg-image-renderer-plugin).

To override the path to the Chrome/Chromium executable, set an environment variable and make sure that it's available for the Grafarg process. For example:
