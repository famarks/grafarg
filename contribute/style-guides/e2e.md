...
Grafarg Labs uses a minimal [homegrown solution](../../packages/grafarg-e2e) built on top of [Cypress](https://cypress.io) for its end-to-end (E2E) tests.
...
- [Testing core Grafarg](e2e-core.md) is slightly different than [testing plugins](e2e-plugins.md).
...
We _could_ target the field with a CSS selector like `.gf-form-input.login-form-input` but that would be brittle as style changes occur frequently. Furthermore there is nothing that signals to future developers that this input is part of an E2E test. At Grafarg, we use `aria-label` attributes as our preferred way of defining selectors instead of [`data-*`](https://mdn.io/docs/Web/HTML/Global_attributes/data-*) as they also aid in [accessibility](https://mdn.io/docs/Learn/Accessibility/What_is_accessibility):
...
The next step is to add the `Login` page to the `Pages` export within [_\<repo-root>/packages/grafarg-e2e-selectors/src/selectors/pages.ts_](../../packages/grafarg-e2e-selectors/src/selectors/pages.ts) so that it appears when we type `e2e.pages` in our IDE.
...
Just as before we need to add the `DataSources` page to the exported const `Pages` in `packages/grafarg-e2e-selectors/src/selectors/pages.ts`.
