...
if [ $PERMISSIONS_OK -eq 1 ]; then
    echo "You may have issues with file permissions, more information here: http://docs.grafarg.org/installation/docker/#migrate-to-v51-or-later"
fi
...
if [ ! -z "${GF_INSTALL_PLUGINS}" ]; then
  ...
  for plugin in ${GF_INSTALL_PLUGINS}; do
    ...
    if [[ $plugin =~ .*\;.* ]]; then
        ...
        grafarg-cli --pluginUrl "${pluginUrl}" --pluginsDir "${GF_PATHS_PLUGINS}" plugins install ${pluginWithoutUrl}
    else
